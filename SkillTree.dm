...
"MagicT4"=list(
			...
			"/obj/Skills/Buffs/SlotlessBuffs/Magic/Resilient_Sphere"=160,
...
mob
	verb
		...
		Buy_Stances()
			set hidden=1
			var/list/obj/Skills/Buffs/NuStyle/Options=list()
			for(var/obj/Skills/Buffs/NuStyle/ns in usr.Buffs)
				if(ns.Mastery<4)
					Options.Add(ns)
			Options.Add("Cancel")
			var/obj/Skills/Buffs/NuStyle/StyleChosen=input(usr, "What style would you like to learn a stance for?", "Stances") in Options
			if(StyleChosen=="Cancel")
				return
			if(StyleChosen.Mastery>=4)
				return
			if(usr.GetRPPSpendable()<(2**(StyleChosen.SignatureTechnique+1)*10))
				usr << "You don't have enough RPP to unlock a new style of [StyleChosen]. ([usr.GetRPPSpendable()]/[(2**(StyleChosen.SignatureTechnique+1)*10)])"
				return


			var/Choice=src.StanceUnlock(StyleChosen)
			if(!Choice)
				return

			if(usr.SpendRPP(2**(StyleChosen.SignatureTechnique+1)*10, "the [Choice] Stance of [StyleChosen]", Training=1))
				usr.StyleUnlock(StyleChosen)
...
mob/proc
	PoweredFormSetup()
		...
		var/list/Stats=list("Strength", "Endurance", "Force", "Speed")
		var/Prime=input(src, "Choose a primary stat for activated Ki Control to buff.", "Powered Form") in Stats
		Stats.Remove(Prime)
		switch(Prime)
			if("Strength")
				KC.StrMult=1.6
			...
			if("Speed")
				KC.SpdMult=1.6
		var/Second=input(src, "Choose a secondary stat for activated Ki Control to buff.", "Ki Control") in Stats
		Stats.Remove(Second)
		switch(Second)
			if("Strength")
				KC.StrMult=1.4
			...
			if("Speed")
				KC.SpdMult=1.4
		var/Tert=input(src, "Choose a tertiary stat for activated Ki Control to buff.", "Ki Control") in Stats
		Stats.Remove(Tert)
		switch(Tert)
			if("Strength")
				KC.StrMult=1.2
			...
			if("Speed")
				KC.SpdMult=1.2
		var/Nerf=input(src, "You shouldn't even see this dialogue box, and if you do it means I've done something wrong.", "Ki Control") in Stats
		Stats.Remove(Nerf)
		switch(Nerf)
			if("Strength")
				KC.StrMult=0.8
