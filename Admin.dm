var/list
	CodedAdmins=list("KokoroWatari"=4, "Kokutou"=4,"Galion"=4, "Niezan"=4)
	Admins=new
	Mappers=new
	Punishments=new
...
mob/Admin3/verb
	...
	Potential_Boost(var/mob/m in world, var/val as num|null)
		set category="Admin"
		if(val&&m)
			m.Potential+=val
			m.PotentialCap+=val
	...
	SetDeadSpawn()
		...
		if(NewLoc)
			...
			global.DeadY=Y
			global.DeadZ=Z
			Log("Admin", "[ExtractInfo(usr)] set the new dead spawn to ([global.DeadX], [global.DeadY], [global.DeadZ])")
		else
	...
	Wound_Lightly(var/mob/m in world)
		set category="Admin"
	...
	Wound_Heavily(var/mob/m in world)
		set category="Admin"
		var/Time=RawHours(12)
		Time/=m.GetRecov()
		m.BPPoisonTimer=Time
...
mob/Admin1/verb
	Tech_Unlock(mob/Players/m in world)
		set category="Admin"
		...
		switch(Mode)
			if("Add")
				...
				"Spell Focii", "Artifact Manufacturing", "Arcane Espionage", "Flying Devices", "Warding Glyphs",
			...
			if("Remove")
				...
				if(Choice=="Cancel")
					return
				m.UnlockedTechnology.Remove(Choice)
...
TO BE CORRECTED
	...
	Message_Global(msg as message)
				...
				switch(input("IC Announcement or OOC Announcement?") in list("IC", "OOC"))
					if("IC")
						client.HttpPost(
							...
							"https://discordapp.com/api/webhooks/719570174377984001/JDpAQTgxDj8eozZ2zOueL6lWbJHQRRVcB6ebgpb1iDcjD2y2xZ8zVHTuO8ekFHyYiFE3",
							list(
					...
					if("OOC")
						client.HttpPost(
							...
							"https://discordapp.com/api/webhooks/719570023672447056/mS9zYs6eKLzAH9p-OsleSNs-d7qXDEWxsvEmG3H8imHBwjvX-2S7iK3pkiQtAChJXxjk",
							list(
								content = discord_output,
		...
		for(var/mob/M in world)if(M.Admin)M<<"[usr] used message (global)."
...
//		A.TotalCapacity=0
	...
	AdminHealCapacity(var/mob/m in world)
		set category="Admin"
		m.TotalCapacity=0
	...
	Ban(mob/M in world)
		...
		var/Duration=99999999999999//=input("Ban Duration?(IN HOURS)","Rebirth")as num
	...
	Give_Make(mob/A in world)
		...
		blah+="[A]<br>[A.type]"
		blah+="<table width=10%>"
		switch(input("") in list("Skills","Items","Object","Mob","Turf","Cancel"))
			if("Skills") B.Add(typesof(/obj/Skills))
			if("Items") B.Add(typesof(/obj/Items))
	...
	CreateSpawnPoint()
		...
		if(confirm=="Yes")
			...
			var/races=input("Pick a race to add to the new spawn point!") in list("Cancel","All", "Human", "Half Saiyan", "Makyo", "Namekian", "Saiyan", "Tuffle", "Monster", "Shinjin", "Demon", "Djinn", "Dragon", "Alien", "Android", "Changeling")
			if(races=="Cancel")
				del(SP)
				return
			else
	...
	RPP_Set_Starting(var/val as num|null)
		...
		if(val<=0 || val==null)
			return
		RPPStarting=val
		Log("Admin", "[ExtractInfo(usr)] set the global RPP starting value to [RPPStarting]. It will be acquired in [RPPStartingDays] days after making a new character.")
	...
	RPP_Set(mob/Players/P in world)
		set category="Admin"
		set hidden=1
	...
	RPP_Event_Give(var/mob/Players/P in world)
		set category="Admin"
	...
	RPP_Refund(mob/Players/P in world)
		set category="Admin"
		var/Refund=0
		...
		P.RPPSpent-=Refund
		P.Potential-=Refund/global.RPPDaily
		P.PotentialDailyGain+=Refund/global.RPPDaily
	...
	RPP_Everyone()
		...
		for(var/mob/Players/P in world)
			...
			if(YourRPP>0)
				if(locate(/obj/Skills/Utility/Teachz, P))
					...
					if(P.EraBody=="Senile"||P.Race=="Shinjin")
						ElderMult=1
					P.RPPDonate+=(YourRPP*ElderMult*P.RPPMult*global.RPPBaseMult)
			...
			if((P.EraBody!="Child"||!P.EraBody)&&!P.Dead)
				...
				if(P.EraBody=="Senile")
					AgeMult=0.25
				P.GiveMoney(round(global.EconomyIncome*P.EconomyMult*P.Intelligence*AgeMult))
		global.RPPStarting+=AddRPP
		Log("Admin", "[ExtractInfo(src)] triggered routine RPP gains of [Commas(AddRPP)].")

	RPP_Routine(mob/Players/P in world)
		set category="Admin"
		set hidden=1
		var/AddRPP=global.RPPDaily
		var/YourRPP=AddRPP
	...
	RPP_Equalize_Everyone()
		...
		for(var/mob/Players/P in world)
			...
			if(YourRPP>0)
				if(locate(/obj/Skills/Utility/Teachz, P))
					...
					if(P.EraBody=="Senile"||P.Race=="Shinjin")
						ElderMult=1
					P.RPPDonate=(Cap*ElderMult*P.RPPMult*global.RPPBaseMult)
					P << "You have gained knowledge on how to help further other's development!"
		...
		global.RPPStarting=Cap
		Log("Admin", "[ExtractInfo(src)] triggered RPP equalization.")

	RPP_Equalize_Target(mob/Players/P in world)
		set category="Admin"
		set hidden=1
	...
	New_Character_Setup(mob/Players/M in world)
		set category="Admin"
		set hidden=1
		if(locate(/obj/Skills/Utility/Teachz, M))
			...
			if(M.EraBody=="Senile"||M.Race=="Shinjin")
				ElderMult=1
			M.RPPDonate+=(global.RPPStarting*ElderMult*M.RPPMult*global.RPPBaseMult)
			M << "You have gained knowledge on how to help further other's development!"

		var/EMult=global.RPPBaseMult
		EMult*=M.GetRPPMult()
		M.RPPSpendable=(global.RPPStarting*EMult)
		Log("Admin", "[ExtractInfo(src)] triggered [ExtractInfo(M)]'s starter rewards!")

	Event_Character_Setup(mob/Players/M in world)
		set category="Admin"
		var/EMult=global.RPPBaseMult
		EMult*=M.GetRPPMult()
		M.RPPSpendable=(global.RPPStarting*EMult*1.5)
		M.PotentialRate=0
	...
	Mastery_Set(mob/Players/P in world)
		set category="Admin"
...
mob/Admin3/verb
	...
	Give_Mapper(var/mob/m in world)
		set category="Admin"
		m.Admin("GiveMapper")
		Log("Admin", "[ExtractInfo(usr)] has made [ExtractInfo(m)] into a mapper!")
	Remove_Mapper(var/mob/m in world)
		set category="Admin"
	...
	MakeResourceSpot()
		...
		RS.loc = src.loc
		RS.Value=val
		if(RS.Value==global.EconomyIncome*2)
			RS.transform*=2
			RS.suffix="(Small)"
		else if(RS.Value==global.EconomyIncome*4)
			RS.transform*=2.5
			RS.suffix="(Moderate)"
		else if(RS.Value==global.EconomyIncome*6)
			RS.transform*=3
			RS.suffix="(Large)"
		else
			RS.transform*=5
	...
	Give_Money(var/mob/m in world, var/num as num)
		set category="Admin"
		set name="Give Money"
		var/Highest=global.EconomyMult
	...
	LockedRacesOptions()
		...
		if(blah=="View")
			for(var/x in LockedRaces)
				for(var/e in x)
					usr<<"[e] : [x[e]]"
		if(blah=="Add")
			var/unlock=input("Add to what list?","Locked Races") in list("Half Saiyan", "Shinjin", "Demon", "Djinn", "Dragon", "Changeling")
			if(unlock)
		...
		if(blah=="Add All")
			...
			if(keytounlock)
				...
				LockedRaces.Add(list(params2list("Djinn=[keytounlock]")))
				LockedRaces.Add(list(params2list("Dragon=[keytounlock]")))
				LockedRaces.Add(list(params2list("Changeling=[keytounlock]")))
		if(blah=="Remove")
			var/unlock=input("Remove from what list?","Locked Races") in list("Half Saiyan", "Shinjin", "Demon", "Djinn", "Dragon", "Changeling")
			if(unlock)
	...
	Set_Base()
		set category="Admin"
		var/NewBase=input(usr,"Set base battle power to what?  Currently [Commas(WorldBaseAmount)]") as num
		WorldBaseAmount=NewBase
		for(var/mob/Players/P in world)
			P.Base=WorldBaseAmount
...
mob/Admin4/verb
	Wipe_Start()
		...
		switch(alert(usr, "Are you sure you want to set the start time of the wipe to midnight today?", "Are you sure you want to suffer through another wipe?", "No", "Hell No", "I guess"))
			...
			if("Hell No")
				return
		global.DaysOfWipe=0
		global.WipeStart=world.realtime-world.timeofday
	...
	Wipe_Restart()
		...
		if(val&&val>0)
			global.DaysOfWipe=0
			global.WipeStart=Today()-Day(val)
			global.RPPStarting=global.RPPDaily*DaysOfWipe()
	...
	Potential_Daily_Set()
		...
		if(val&&val>0)
			global.PotentialDaily=val
			Log("Admin", "[ExtractInfo(src)] has set the daily potential rate to [global.PotentialDaily].")
	RPP_Highest_Set()
		set category="Admin"
		var/val=input(src, "What should the highest RPP be set to?", "RPP Highest") as num|null
		if(val&&val>0)
			global.RPPHighest=val
			Log("Admin", "[ExtractInfo(src)] has set the RPP Highest to [global.RPPHighest].")
	Era_Set(var/val as num)
		set category="Admin"
		global.Era=val
		Log("Admin", "[ExtractInfo(src)] set the Era to [global.Era].")
	RPP_Limit_Set(var/val as num)
		set hidden=1
	RPP_Base_Mult_Set(var/val as num)
		set category="Admin"
		global.RPPBaseMult=val
		Log("Admin", "[ExtractInfo(src)] set the RPP Base Mult to [global.RPPBaseMult]x.")
	RPP_Daily_Set(var/val as num)
		set category="Admin"
		global.RPPDaily=val
		Log("Admin", "[ExtractInfo(src)] set the daily RPP increment to [Commas(global.RPPDaily)].")
	Rename_Money()
		set category="Admin"
		var/NewMoney=input(usr, "What should money be called?  Currently known as: [global.MoneyName]", "Rename Money") as text|null
		if(NewMoney)
			Log("Admin", "[ExtractInfo(usr)] renamed the currency from [global.MoneyName] to [NewMoney].")
			global.MoneyName=NewMoney
	Common_Toggle()
		set category="Admin"
		var/list/Races=list("Cancel", "Half Saiyan", "Elite", "Giant", "Shinjin", "Demon", "Djinn", "Dragon", "Makyo", "Changeling")
		...
		if(Mode=="Make Common")
			...
			if(Choice!="Cancel")
				...
				if(Confirm=="Yes")
					global.CustomCommons.Remove(Choice)
					Log("Admin", "[ExtractInfo(usr)] made [Choice] rare again!")
	NoVoid(var/mob/m in world)
		set hidden=1
		if(m.NoVoid)
			m.NoVoid=0
	...
	SetGlobalDamage()
		set category="Admin"
		var/m=input(src, "What do you want to set the global damage multiplier to? (currently x[global.WorldDamageMult])", "World Damage Multiplier") as num
		global.WorldDamageMult=m
		Log("Admin", "[ExtractInfo(src)] set Global Damage Mult to [m]!")
	SetDefaultAccuracy()
		set category="Admin"
		var/m=input(src, "What do you want to set the default accuracy to? (currently [global.WorldDefaultAcc]%)", "World Default Accuracy") as num
		global.WorldDefaultAcc=m
		Log("Admin", "[ExtractInfo(src)] set default accuracy to [m]%!")
	SetWhiffRate()
		set category="Admin"
		var/m=input(src, "What do you want the amount of full-powered strikes to be? (currently [global.WorldWhiffRate]%)", "World Whiff Rate") as num
		global.WorldWhiffRate=m
		Log("Admin", "[ExtractInfo(src)] set whiff rate to [m]%!")
	SetVoidChance()
		set category="Admin"
		var/m=input(src, "What do you want to set Void Chance to? (currently [global.VoidChance]%)", "Void Chance") as num
		global.VoidChance=m
		//world << "<font color='green'>Void Chance set to [m]%!</font color>"
		Log("Admin", "[ExtractInfo(src)] set Void Chance to [m]%!")
	ToggleVoid()
		set category="Admin"
		if(global.VoidsAllowed)
			global.VoidsAllowed=0
			world << "<font color='red'>Voiding from death has been disabled.</font>"
		else
			global.VoidsAllowed=1
...
atom/Topic(A,B[])
	if(B["action"]=="edit")
		if(!usr.Admin&&!usr.Mapper) return
		var/variable=B["var"]
