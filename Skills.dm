	...
	Six_Paths_of_Pain
		...
		verb/Deva_Path()
			...
			if(!usr.CanAttack())
				return
			if(!src.Using)
				if(usr.StanceActive=="Basic"&&usr.CheckSlotless("Susanoo"))
					src.Cooldown(2)
					usr.UseProjectile(new/obj/Skills/Projectile/Zone_Attacks/Shattered_Heaven)
				else if(usr.StanceActive in list("Advancing","Striking"))
					var/Distance=20
					...
					if(Tgt&&Tgt in view(16, usr))
						...
						Tgt.Knockbacked=0
						Tgt.icon_state=""
				else if(usr.StanceActive in list("Evasive","Defensive"))
					src.Cooldown(1/12)
					var/Wave=5
		...
		if(locate(/obj/Skills/Teleport/Instant_Transmission,usr.contents))
			if(T) if(T.icon)
				for(var/turf/A in view(0,usr))
					if(A==src)
						return
				if(!T.density&&usr.icon_state!="Mediate"&&!usr.Observing&&(usr.Beaming!=2))
					VanishImage(usr)
