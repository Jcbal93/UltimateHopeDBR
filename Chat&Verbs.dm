...
mob/Players/verb
		...
		switch(input(src, "How do you want to manage your party?", "Manage Party") in options)
			...
			if("Add Member")
				if(src==src.party.leader)
					...
					if(my_boy=="Cancel")
						return
					if(my_boy.icon_state!="Meditate")
						src << "[my_boy] must meditate to be added to the party."
						return
					if(my_boy.party)
						return
	...
	Custom_Object()
		set category="Other"
		set name="Customize: Special Object"
		if(!(world.time > usr.verb_delay)) return
		usr.verb_delay=world.time+1
	...
	Custom_Turf()
		set category="Other"
		set name="Customize: Special Turf"
		if(!(world.time > usr.verb_delay)) return
		usr.verb_delay=world.time+1
	...
	Reset_Multipliers()
		...
		if(!(world.time > usr.verb_delay)) return
		usr.verb_delay=world.time+1
		for(var/x in usr.SlotlessBuffs)
			if(x==null)
				usr.SlotlessBuffs.Remove(x)
				del x
			...
			if(!src.PoseEnhancement)
				if(!src.CheckSlotless("Half Moon Form")&&!src.CheckSlotless("Full Moon Form"))
					if(src.PoseTime>=5&&(src.HasRipple()||src.Secret=="Vampire"||src.Secret=="Senjutsu"&&src.CheckSlotless("Senjutsu Focus")))
						...
						if(src.Secret=="Senjutsu"&&src.CheckSlotless("Senjutsu Focus"))
							...
							if(src.ManaAmount>100)
								for(var/obj/Skills/Buffs/SlotlessBuffs/Autonomous/Sennin_Mode/S in src)
									S.Trigger(src)
				else
					...
					if(src.Target)
						...
						src.Melee(damagemulti=PoseBuff, accmulti=PoseBuff, NoKB=1)
					src.PoseEnhancement=1
					spawn(Second(30))
						src.PoseEnhancement=0
			return
			...
			for(var/mob/Players/P)
				for(var/obj/Communication/C in P)
					if(C.ShowOOC)
						if(P.Timestamp)
							if(C.AllTabOOC)
								P << output("<font color=red>[time2text(world.timeofday,"(hh:mm:ss)")]<font color=lime><b>OOC:</b><font color=[usr.OOC_Color]> <b>[keyjack]</b>[P.Controlz(usr)]: <font color=white>[html_encode(T)]", "output")
							P << output("<font color=red>[time2text(world.timeofday,"(hh:mm:ss)")]<font color=lime><b>OOC:</b><font color=[usr.OOC_Color]> <b>[keyjack]</b>[P.Controlz(usr)]: <font color=white>[html_encode(T)]", "oocchat")
						else
							if(C.AllTabOOC)
								P << output("<font color=lime><b>OOC:</b><font color=[usr.OOC_Color]> <b>[keyjack]</b>[P.Controlz(usr)]: <font color=white>[html_encode(T)]", "output")
							P << output("<font color=lime><b>OOC:</b><font color=[usr.OOC_Color]> <b>[keyjack]</b>[P.Controlz(usr)]: <font color=white>[html_encode(T)]", "oocchat")
	verb/Say(T as text)
		set category="Roleplay"
		set src=usr.contents
		if(!(world.time > usr.verb_delay)) return
		...
		for(var/mob/E in hearers)
			if(LOOCSay||LOOCSay2||LOOCSay3||LOOCSay4||LOOCSay5)
				for(var/obj/Communication/C in E)
					E << output("<font color=[usr.Text_Color]>[usr][E.Controlz(usr)] says: [html_encode(T)]", "loocchat")
					E << output("<font color=[usr.Text_Color]>[usr][E.Controlz(usr)] says: [html_encode(T)]", "oocchat")
				...
				for(var/obj/Items/Tech/Speaker/X in world)
					if(X.Frequency==WT.Frequency&&X.Active==1)
						for(var/mob/Y in hearers(X.AudioRange,X))
							if(!(Y in hearers(12, usr)))
								Y<<"<font color=green><b>([X.name])</b> [usr.name]: [html_encode(T)]"
		...
		for(var/obj/Items/Tech/Speaker/X in view(20,usr)) //This for loop detects Speakers, then determines if they have the Intercom upgrade.
			...
			if(X.Intercom==1)
				for(var/mob/Players/M in world)
					if(M.InMagitekRestrictedRegion()) continue
					for(var/obj/Items/Tech/Scouter/Q in M)
...
//					m<<output("<b>(OBSERVE)</b><font color=[usr.Text_Color]>[usr][E.Controlz(usr)] whispers: [html_encode(T)]", "output")
	...
	verb/Emote()
		...
		for(var/mob/E in hearers)
			E << output("<font color=[usr.Text_Color]>*[usr.name]<font color=yellow>[E.Controlz(usr)] [html_decode(T)]*", "output")
			E << output("<font color=[usr.Text_Color]>*[usr.name]<font color=yellow>[E.Controlz(usr)] [html_decode(T)]*", "icchat")
			if(E.BeingObserved.len>0)
				for(var/mob/m in E.BeingObserved)
					m<<output("<b>(OBSERVE)</b><font color=[usr.Text_Color]>*[usr][E.Controlz(usr)]  [html_decode(T)]*", "icchat")
					m<<output("<b>(OBSERVE)</b><font color=[usr.Text_Color]>*[usr][E.Controlz(usr)]  [html_decode(T)]*", "output")
			if(E==usr)
...
//				spawn()TempLog(E.ChatLog(),"<font color=#CC3300>*[usr]([usr.key]) [html_decode(T)]*")
					...
					for(var/mob/Players/OrbCheck in world)
						for(var/obj/Items/Enchantment/ArcanicOrb/FinalCheck in OrbCheck)
							if(EyeCheck.LinkTag in FinalCheck.LinkedMasks)
								if(FinalCheck.Active)
									OrbCheck << output("[FinalCheck](viewing [E]):<font color=[usr.Text_Color]>*[usr.name]<font color=yellow> [html_decode(T)]*", "output")
									OrbCheck << output("[FinalCheck](viewing [E]):<font color=[usr.Text_Color]>*[usr.name]<font color=yellow> [html_decode(T)]*", "icchat") //Outputs to the Orb owner the emote.
			...
			if(F.Active==1)
				for(var/mob/CC in world)
					...
					for(var/obj/Items/Tech/Scouter/CCS in CC)
						if(F.Frequency==CCS.Frequency)
							if(CC.Timestamp)
								CC << output("<font color=red>[time2text(world.timeofday,"(hh:mm:ss)")]<font color=green>[F.name] transmits:<font color=[usr.Text_Color]>*[usr.name]<font color=yellow>[CC.Controlz(usr)] [html_decode(T)]*", "output")
								CC << output("<font color=red>[time2text(world.timeofday,"(hh:mm:ss)")]<font color=green>[F.name] transmits:<font color=[usr.Text_Color]>*[usr.name]<font color=yellow>[CC.Controlz(usr)] [html_decode(T)]*", "icchat")
							else
								CC << output("<font color=green>[F.name] transmits:<font color=[usr.Text_Color]>*[usr.name]<font color=yellow>[CC.Controlz(usr)] [html_decode(T)]*", "output")
								CC << output("<font color=green>[F.name] transmits:<font color=[usr.Text_Color]>*[usr.name]<font color=yellow>[CC.Controlz(usr)] [html_decode(T)]*", "icchat")
				for(var/obj/Items/Tech/Security_Display/G in world)
					if(G.Password==F.Password)
						if(G.Active==1)
							for(var/mob/H in hearers(G.AudioRange,G))
		...
		for(var/obj/Items/Tech/Recon_Drone/FF in view(11,usr))
			if(FF.who)
				FF.who << output("<font color=red>[time2text(world.timeofday,"(hh:mm:ss)")]<font color=green>[FF.name] transmits:*[usr.name]<font color=yellow> [html_decode(T)]*", "output")
				FF.who << output("<font color=red>[time2text(world.timeofday,"(hh:mm:ss)")]<font color=green>[FF.name] transmits:*[usr.name]<font color=yellow> [html_decode(T)]*", "icchat")

		for(var/obj/Items/Tech/SpaceTravel/Ship/A in view(20,usr)) //This for loop detects ships around those that use the say verb.
			for(var/obj/ShipConsole/B in world)
				if(A.Password==B.Password)
					for(var/mob/C in hearers(20,B))
		...
		for(var/obj/ShipConsole/AA in view(20,usr))
			for(var/obj/Items/Tech/SpaceTravel/Ship/BB in world)
				if(AA.Password==BB.Password&&AA.SpeakerToggle==1)
					for(var/mob/C in hearers(20,BB))
						C << output("<font color=green>([AA.name]'s Internal Camera) *[usr.name]<font color=yellow> [html_decode(T)]*", "output")
						C << output("<font color=green>([AA.name]'s Internal Camera) *[usr.name]<font color=yellow> [html_decode(T)]*", "icchat")
