	...
	ResourceSpot
		...
		Click()
			...
			if(usr.EngineeringUnlocked>=1)
				...
				var/Confirm=alert(usr, "Do you want to build a drill on this resource spot?  It will cost [Commas(round(Cost))] [global.MoneyName].", "Build Drill?", "No", "Yes")
		...
		Minor
			...
			New(var/X, var/Y, var/Z)
				...
				if(!src.Value)
					src.Value=(global.EconomyIncome)
				src.transform*=1
		Small
			suffix="(Small)"
			New(var/X, var/Y, var/Z)
				...
				src.loc=locate(X,Y,Z)
				if(!src.Value)
					src.Value=(global.EconomyIncome*2)
				src.transform*=2
		Moderate
			suffix="(Moderate)"
			New(var/X, var/Y, var/Z)
				...
				src.loc=locate(X,Y,Z)
				if(!src.Value)
					src.Value=(global.EconomyIncome*4)
				src.transform*=2.5
		Large
			suffix="(Large)"
			New(var/X, var/Y, var/Z)
				...
				src.loc=locate(X,Y,Z)
				if(!src.Value)
					src.Value=(global.EconomyIncome*6)
				src.transform*=3
		Major
			suffix="(Major)"
			New(var/X, var/Y, var/Z)
				...
				src.loc=locate(X,Y,Z)
				if(!src.Value)
					src.Value=(global.EconomyIncome*30)
				src.transform*=5
		proc
			GetValue(var/Insight)
				if(Insight>=5)
					return "[Commas(src.Value)]"
				if(src.Value<=global.EconomyIncome)
					return "Minor"
				if(src.Value<=global.EconomyIncome*2)
					return "Small"
				if(src.Value<=global.EconomyIncome*4)
					return "Moderate"
				if(src.Value<global.EconomyIncome*6)
					return "Large"
				if(src.Value>=global.EconomyIncome*30)
					return "Major"
			...
			NuDrill
					...
					if(src in range(1, usr))
						for(var/obj/ResourceSpot/RS in range(1, src))
							if(world.realtime>src.InternalTimer+Hour())
								...
								OMsg(usr, "[usr] extracts [Commas(round(Extract))] [global.MoneyName] from the [src]!")
				...
				verb/Dismantle()
					...
					if(locate(/obj/Skills/Utility/Smelt, usr))
						usr.GiveMoney(0.5*global.EconomyCost)
