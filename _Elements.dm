			...
			switch(Attack)
				...
				if("Poison")
					if(!Defender.HasVenomImmune()&&Defense!="Poison")
						Defender.AddPoison(4*DebuffIntensity, Attacker)
				if("Fire")
	...
	proc
		AddBurn(var/Value, var/mob/Attacker=null)
			...
			if(Attacker&&Attacker.HasDarknessFlame()&&Attacker!=src)
				...
				src.AddPoison(Value/2, Attacker=Attacker)
				if(Attacker.IsEvil())
					Attacker.CursedWounds-=1
			...
			for(var/obj/Items/Gear/Automated_Aid_Dispenser/AD in src)
				if(AD.suffix&&AD.Uses)
					...
					if(!src.Antivenomed)
						OMsg(src, "<font color='[rgb(104, 153, 251)]'>[src]'s dispenser deploys a healing mist!!</font color>")
					src.Antivenomed+=100
		AddConfuse(var/Value, var/mob/Attacker=null)
			src.Confused+=Value
			if(src.Confused>100)
				src.Confused=100
			for(var/obj/Items/Gear/Automated_Aid_Dispenser/AD in src)
				if(AD.suffix&&AD.Uses)
					...
					if(!src.Stabilized)
						OMsg(src, "<font color='[rgb(104, 153, 251)]'>[src]'s dispenser deploys a healing mist!!</font color>")
					src.Stabilized+=100
		AddSheared(var/Value, var/mob/Attacker=null)
			if(src.Race=="Djinn")
				if(!src.AscensionsAcquired||src.AscensionsAcquired>=3)
					Value=0
			Value = Value*(1-(src.Sheared/125))
			src.Sheared+=Value
			if(src.Sheared>100)
				src.Sheared=100
			for(var/obj/Items/Gear/Automated_Aid_Dispenser/AD in src)
				if(AD.suffix&&AD.Uses)
					...
					if(!src.Sprayed)
						OMsg(src, "<font color='[rgb(104, 153, 251)]'>[src]'s dispenser deploys a healing mist!!</font color>")
					src.Sprayed+=100
		AddCrippled(var/Value, var/mob/Attacker=null)
			if(src.Race=="Djinn")
				if(!src.AscensionsAcquired||src.AscensionsAcquired>=3)
					Value=0
			if(!src.HasLegendaryPower() > 0.75)
				Value = Value*(1-(src.Crippled/125))
				src.Crippled+=Value
			if(src.Crippled>100)
				src.Crippled=100
			for(var/obj/Items/Gear/Automated_Aid_Dispenser/AD in src)
				if(AD.suffix&&AD.Uses)
					...
					if(!src.Sprayed)
						OMsg(src, "<font color='[rgb(104, 153, 251)]'>[src]'s dispenser deploys a healing mist!!</font color>")
					src.Sprayed+=100
		AddAttracted(var/Value, var/mob/m)
			src.Attracted+=Value
			src.AttractedTo=m
			if(src.Attracted>100)
				src.Attracted=100
		AddTerrified(var/Value, var/mob/m)
			src.Terrified+=Value
			src.TerrifiedOf=m
...
mob
	proc
		Debuffs()
			...
			if(src.Poison)
				if(src.Poison>0)
					if(src.Antivenomed)//Halved damage
						src.DoDamage(src, TrueDamage(5/15*(src.Poison/100)*0.25))
						if(!src.Desperation)
							src.WoundSelf(5/15*(src.Poison/100)*0.25)
					else
						src.DoDamage(src, TrueDamage(5/15*(src.Poison/100)*0.5))
						if(!src.Desperation)
							src.WoundSelf(5/15*(src.Poison/100)*0.5)
				if(src.Antivenomed||src.Infusion||src.VenomResistance)
					if(src.VenomResistance)
						src.Poison-=(src.GetRecov(0.25))*(1+src.GetDebuffImmune()+src.VenomResistance)
					else
						src.Poison-=(src.GetRecov(0.25))*(1+src.GetDebuffImmune())
				else
					src.Poison-=src.GetRecov(0.25)*(1+src.GetDebuffImmune())
				if(src.Poison<=0)
					src.Poison=0
			...
			if(src.Burn)
				if(src.Burn>0)
					if(src.Cooled)
						src.DoDamage(src, TrueDamage(1.25/15*(src.Burn/25)*0.5))
						if(!src.Desperation)
							src.WoundSelf((1.25/15*(src.Burn/25)*0.25))
					else
						src.DoDamage(src, TrueDamage(1.25/15*(src.Burn/25)))
						if(!src.Desperation)
							src.WoundSelf(1.25/15*(src.Burn/25)*0.5)
				src.Burn-=(src.GetEnd(0.25)+src.GetRecov(0.25))*(1+src.GetDebuffImmune())
				if(src.Burn<=0)
					src.Burn=0

			if(src.Shatter)
				if(src.Sprayed)
					src.Shatter-=(src.GetEnd(0.25)+src.GetRecov(0.25))*2*(1+src.GetDebuffImmune())
				else
					src.Shatter-=(src.GetEnd(0.25)+src.GetRecov(0.25))*(1+src.GetDebuffImmune())
				if(src.Shatter<=0)
					src.Shatter=0

			if(src.Slow)
				if(src.Cooled)
					src.Slow-=(src.GetEnd(0.25)+src.GetRecov(0.25))*2*(1+src.GetDebuffImmune())
				else
					src.Slow-=(src.GetEnd(0.25)+src.GetRecov(0.25))*(1+src.GetDebuffImmune())
				if(src.Slow<=0)
					src.Slow=0

			if(src.Shock)
				if(src.Stabilized)
					src.Shock-=(src.GetEnd(0.25)+src.GetRecov(0.25))*2*(1+src.GetDebuffImmune())
				else
					src.Shock-=(src.GetEnd(0.25)+src.GetRecov(0.25))*(1+src.GetDebuffImmune())
				if(src.Shock<=0)
					src.Shock=0

			if(src.Crippled)
				if(src.Sprayed)
					src.Crippled-=(src.GetEnd(0.25)+src.GetRecov(0.25))*2*(1+src.GetDebuffImmune())
				else
					src.Crippled-=(src.GetEnd(0.25)+src.GetRecov(0.25))*(1+src.GetDebuffImmune())
				if(src.Crippled<=0)
					src.Crippled=0
