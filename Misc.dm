mob/Click()
	if(usr.Target!=src)
		for(var/obj/Skills/Buffs/b in usr.SlotlessBuffs)
			if(b.TargetOverlay)
...
proc/Observify(mob/P,atom/A)
	if(P.client.perspective==MOB_PERSPECTIVE|EDGE_PERSPECTIVE)
		P.client.perspective=EYE_PERSPECTIVE|EDGE_PERSPECTIVE
	if(P.Observing)
	...
	if(A!=P)
		...
		if(P.client.perspective==EYE_PERSPECTIVE|EDGE_PERSPECTIVE)
