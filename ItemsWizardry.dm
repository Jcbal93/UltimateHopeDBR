...
obj/Items/Enchantment
					...
					switch(Mode)
						if("Transmute Lifeforce")
							...
							if(Choice=="Self"||Choices.len<=2)
								...
								if(Amt&&Amt>0)
									var/Cost=Amt*(global.EconomyCost/100)
									var/Confirm=alert(usr, "Restoring [round(Amt)] capacity will cost [Commas(round(Cost))] [MoneyName].  Do you want to spend it?", "Restore Capacity", "No", "Yes")
									if(Confirm=="No")
										src.Using=0
										return
						...
						if("Enhance Potion")
							...
							var/MaxEffect=1
							if("Distillation Process" in usr.UnlockedTechnology)
								MaxEffect=2
							if(Choice.Gimmick)
	...
	Potion
		...
		Click()
			...
			if(usr.is_arcane_beast)
				...
				if(src.Energy)
					PP.InstantAffect=1
					PP.EnergyHeal=src.Energy*25
					CD+=(src.Energy*180)
				if(src.Mana)
					PP.InstantAffect=1
					PP.ManaHeal=src.Mana*25
					CD+=(src.Mana*180)
				if(src.Heal)
					PP.InstantAffect=1
					PP.StableHeal=1
				...
				if(src.Searing)
					FoundSearing=1
					PP.PureDamage=src.Searing
					CD+=(src.Searing*90)
				if(src.Hard)
					FoundHard=1
					PP.Hardening=src.Hard*2
					CD+=(src.Hard*90)
				if(src.Hallucinogen)
					FoundHallucinogen=1
					PP.AutoAnger=1
					PP.AngerMult=1+0.5*(src.Hallucinogen-1)
					for(var/x=0, x<src.Hallucinogen, x++)
						PP.RegenMult-=0.375
					CD+=(src.Hallucinogen*180)
				if(src.Flowy)
					FoundFlowy=1
					PP.Flow=0.5*src.Flowy
					CD+=(src.Flowy*180)

				if(src.Sexy)
					FoundSexy=1
					PP.Infatuated=src.Sexy*2
					CD+=(src.Sexy*90)
				if(src.Transform)
				...
				if(src.Toxic)
					PP.InstantAffect=1
					PP.PoisonAffected=src.Toxic*25
	...
	Crystal_Ball
		...
		EnchType="ToolEnchantment"
		SubType="Artifact Manufacturing"
		Cost=100
		Health=10
		Grabbable=1
		Pickable=1
	...
	Flying_Device
		EnchType="ToolEnchantment"
		SubType="Flying Devices"
		icon='Flying Nimbus.dmi'
		...
		verb/Set_Flying_State()
			set category=null
			set src in usr
			src.CustomState=input(usr, "What icon state should your flying device set you in?", "Set Flying State") as text|null
	...
	Tome
		...
		verb/Examine()
			...
			for(var/obj/Skills/x in src.Spells)
				content+="<tr><td>[x]</td><td>Level: [x.Copyable]</td></tr>"
			content+="</table>"
		...
		verb/Manage_Contents()
			...
			if((usr.TomeCreationUnlocked+usr.CrestCreationUnlocked)*usr.Imagination*10>src.Level)
				Options.Add("Expand")
			if("Tome Cleansing" in usr.UnlockedTechnology)
				Options.Add("Cleanse")
			if("Tome Security" in usr.UnlockedTechnology)
				Options.Add("Secure")
			if("Tome Translation" in usr.UnlockedTechnology)
				Options.Add("Study")
			if("Tome Binding" in usr.UnlockedTechnology)
				Options.Add("Bind")
			if("Tome Excerpts" in usr.UnlockedTechnology)
	...
	Dimensional_Cage
		...
		EnchType="SpaceMagic"
		SubType="Dimensional Restriction"
		icon='Caja.dmi'
		Cost=100
		var
			ReturnX=0
	...
	PocketDimensionGenerator
		...
		EnchType="SpaceMagic"
		SubType="Dimensional Manipulation"
		Cost=200
		Destructable=0
		verb/SetPassword()
			set src in view(1, usr)
