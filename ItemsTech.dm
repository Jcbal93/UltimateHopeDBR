...
obj/Items/Tech
	...
	First_Aid_Kit
		...
		Click()
			if(!(src in usr))
				...
				if(Choice.KO)
					Choice.Conscious()
				var/heal=0.25*Choice.TotalInjury
	...
	Revitalization_Serum
		...
		Click()
			if(!(src in usr))
				...
				usr.NextSerum=world.realtime+Day(1)
	...
	Youth_Serum
		...
		icon='Tech.dmi'
		icon_state="Youth"
		desc="A serum that can be used to instantly imbue the body with youthful energy, though it can't be used once the body has already passed its prime."
		Cost=50
		Click()
			if(!(src in usr))
				..()
				...
				if(src.Using)
					usr << "You're already preparing a youth serum!"
					return
				if(usr.Race=="Android")
					return
				if(usr.icon_state!="Meditate")
					...
					return
				src.Using=1
				var/Choice=alert(usr, "Do you want to inject a youth serum into yourself?  This will give you more time to live, but weaken you until you age up again...", "Youth Serum", "No", "Yes")
				if(Choice=="No")
					src.Using=0
					return
				if(usr.EraBody=="Elder")
					usr << "It's too late for you to age yourself down."
					src.Using=0
					return
				usr.EraAge+=1
				switch(usr.EraBody)
					...
					if("Youth")
						...
						if(usr.Race=="Saiyan"||usr.Race=="Half Saiyan")
							usr.Tail(1)
				OMsg(usr, "[usr] injects themselves with a youth serum, instantly growing younger!")
				src.Using=0
				del src
	...
	Security_Display
		...
		verb/Download_Techniques()
			...
			if(src.Password)
				...
				if(PassCheck!=src.Password)
					...
					return
			if(!("Combat Scanning" in usr.UnlockedTechnology))
		...
		verb/ViewCamera()
			...
			if(pickcamera=="Cancel")
				if(usr.client.perspective!=MOB_PERSPECTIVE|EDGE_PERSPECTIVE)
					usr.client.perspective=MOB_PERSPECTIVE|EDGE_PERSPECTIVE
				usr.client.eye=usr
			else
				if(usr.client.perspective!=EYE_PERSPECTIVE|EDGE_PERSPECTIVE)
							...
							if(locate(/obj/Skills/Buffs/SlotlessBuffs/Werewolf/Full_Moon_Form, m))
								if(!m.CheckSlotless("FullMoonForm"))
									...
									if(m.SlotlessBuffs.len>0)
										for(var/obj/Skills/Buffs/b in m.SlotlessBuffs)
											b.Trigger(m)
				...
				if("Ultraviolet")
					...
					for(var/turf/t in Turf_Circle(src, 10))
						...
						for(var/mob/m in t)
							switch(m.Secret)
								if("Vampire")
									m.BPPoison=min(0.2*m.BloodPower,0.9)
									m.BPPoisonTimer=RawHours(6)/m.BloodPower
								if("Ripple")
	...
	Charging_Station
		...
		proc/Recharging()
			set background=1
			sleep(6000/src.Level)
			src.ChargesTotal=100
			src.icon_state="5"

	Android_Frame
		TechType="CyberEngineering"
		SubType="Android Creation"
		Cost=25
	...
	Chip_Controller
		Health=5
		TechType="CyberEngineering"
		SubType="Involuntary Implantation"
		Cost=5
		icon='device.dmi'
...
obj/Items/Gear
	...
	verb/Upgrade()
		...
		if(src.suffix=="*Equipped*")
			...
			return
		if(!(src.SubType in usr.UnlockedTechnology))
		...
		var/obj/Items/I=new src.UpgradePath
